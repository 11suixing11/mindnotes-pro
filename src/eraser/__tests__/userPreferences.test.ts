...
describe('userPreferences - 用户偏好持久化', () => {
  ...
  describe('清除功能', () => {
    it('clearEraserPreferences应该清除所有保存的偏好', () => {
      saveEraserPreferences({ preset: '6b', shape: 'chisel' })
      expect(loadEraserPreferences().preset).toBe('6b')
  ...
  describe('配置生成', () => {
    it('应该根据偏好生成正确的橡皮擦配置', () => {
      ...
      // 预设的硬度等参数应该来自2B配置
      expect(config.hardness).toBe(ERASER_PRESET_CONFIGS['2b'].hardness)
      expect(config.wearRate).toBe(ERASER_PRESET_CONFIGS['2b'].wearRate)
    })
    ...
    it('4B预设应该生成对应配置', () => {
      ...
      const config = getEraserConfigFromPreferences(prefs)
      expect(config.hardness).toBe(ERASER_PRESET_CONFIGS['4b'].hardness)
    })
    ...
    it('6B预设应该生成对应配置', () => {
      ...
      const config = getEraserConfigFromPreferences(prefs)
      expect(config.hardness).toBe(ERASER_PRESET_CONFIGS['6b'].hardness)
    })
  })
  ...
  describe('边界情况', () => {
    ...
    it('保存undefined值应该被忽略', () => {
      saveEraserPreferences({ preset: '6b' })
      saveEraserPreferences({ preset: undefined as any })
