import type { CanvasElement, StrokeElement, ShapeElement, TextElement, ImageElement, BrushType } from '../store/types'
import { getImage } from './canvasUtils'
import getStroke from 'perfect-freehand'

export function drawElement(ctx: CanvasRenderingContext2D, el: CanvasElement, isDarkMode: boolean, editingTextId?: string) {
...
export function drawStrokeEl(ctx: CanvasRenderingContext2D, el: StrokeElement, isDarkMode: boolean) {
  ...
  if (b === 'pen') {
    ...
    ctx.save(); ctx.globalAlpha = el.opacity ?? 0.65; ctx.strokeStyle = el.color; ctx.lineWidth = el.size * 0.6; ctx.lineCap = 'round'
    ...
    ctx.shadowColor = el.color; ctx.shadowBlur = el.size * glowMultiplier
    ctx.strokeStyle = el.color; ctx.lineWidth = el.size * 0.4; ctx.globalAlpha = alphaBoost
    ctx.beginPath(); ctx.moveTo(pts[0][0], pts[0][1])
    for (let i = 1; i < pts.length; i++) { const p = pts[i - 1], c = pts[i]; ctx.quadraticCurveTo(p[0], p[1], (p[0] + c[0]) / 2, (p[1] + c[1]) / 2) }
    ctx.stroke()
    ctx.shadowBlur = el.size * glowMultiplier * 0.5; ctx.lineWidth = el.size * 0.7; ctx.globalAlpha = alphaBoost * 0.6
    ctx.beginPath(); ctx.moveTo(pts[0][0], pts[0][1])
    for (let i = 1; i < pts.length; i++) { const p = pts[i - 1], c = pts[i]; ctx.quadraticCurveTo(p[0], p[1], (p[0] + c[0]) / 2, (p[1] + c[1]) / 2) }
    ctx.stroke(); ctx.restore()
  }
...
export function drawStrokeRaw(ctx: CanvasRenderingContext2D, pts: number[][], c: string, s: number, b: BrushType, isDarkMode: boolean) {
  drawStrokeEl(ctx, { type: 'stroke', id: '', points: pts, color: c, size: s, brush: b }, isDarkMode)
}
...
export function drawSelBox(ctx: CanvasRenderingContext2D, b: { x: number; y: number; w: number; h: number }, isDarkMode: boolean, zoom: number) {
  const primary = isDarkMode ? '#C8A0B0' : '#B07D6E'
  const primaryLight = isDarkMode ? 'rgba(200,160,176,0.12)' : 'rgba(176,125,110,0.1)'

  ctx.save()
  ctx.strokeStyle = primary; ctx.lineWidth = 1.5 / zoom; ctx.setLineDash([5 / zoom, 5 / zoom])
...
export function drawMonetGrid(ctx: CanvasRenderingContext2D, viewBox: { x: number; y: number; zoom: number }, canvasSize: { w: number; h: number }, isDarkMode: boolean) {
...
export function drawCanvasBackground(ctx: CanvasRenderingContext2D, canvasSize: { w: number; h: number }, bgColor: string, isDarkMode: boolean) {
...
export function drawMinimap(ctx: CanvasRenderingContext2D, elements: CanvasElement[], cachedBounds: (el: CanvasElement) => { x: number; y: number; w: number; h: number }, viewBox: { x: number; y: number; zoom: number }, canvasSize: { w: number; h: number }, isDarkMode: boolean) {
...
export function drawZoomLevel(ctx: CanvasRenderingContext2D, viewBox: { zoom: number }, canvasSize: { w: number; h: number }, isDarkMode: boolean, dpr: number) {
