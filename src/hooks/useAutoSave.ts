...
export function useAutoSave() {
  ...
  const restore = useCallback(async () => {
    try {
      ...
      const { strokes, shapes, viewBox } = saved
      const store = useAppStore.getState()

      // 只在画布为空时恢复（避免覆盖用户新内容）
      if (store.strokes.length > 0 || store.shapes.length > 0) return false
  ...
  const save = useCallback(async () => {
    try {
      const { strokes, shapes, viewBox } = useAppStore.getState()
  ...
  useEffect(() => {
    const unsubscribe = useAppStore.subscribe((state) => {
      // 只在有内容时保存
      if (state.strokes.length === 0 && state.shapes.length === 0) return
      ...
      if (strokes.length > 0 || shapes.length > 0) {
        ...
        try {
          localStorage.setItem(
            'autosave-sync',
            JSON.stringify({ strokes, shapes, viewBox, timestamp: Date.now() })
          )
        } catch {
          // 忽略
        }
