...
export default function CanvasPreview({ elements, width = 56, height = 36, bgColor = '#F6F0E6' }: Props) {
    ...
    for (const el of elements) {
      if (el.type === 'stroke' && el.points.length >= 2) {
        ...
        if (el.kind === 'rectangle') {
          if (hasFill) { ctx.fillStyle = el.fillColor!; ctx.fillRect(el.x, el.y, el.w, el.h) }
