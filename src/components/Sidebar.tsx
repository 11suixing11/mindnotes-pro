  ...
  if (!sidebarOpen) {
    return (
      <button
        ...
        style={{ position: 'fixed', left: 8, top: '50%', transform: 'translateY(-50%)', zIndex: 25 }}
      >☰</button>
    )
  }
      ...
      >
        ...
        {renamingId === doc.id ? (
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') handleRename(doc.id, 'doc'); if (e.key === 'Escape') setRenamingId(null) }}
            onClick={(e) => e.stopPropagation()}
            className="sb-rename-input"
            style={{ borderRadius: 6, padding: '2px 6px' }}
          />
        ) : (
          <div className="sb-doc-content">
            <div className={`sb-doc-title${isActive ? ' sb-doc-title-active' : ''}`}>{doc.title}</div>
  ...
  const renderFolder = (folder: CanvasFolder, depth: number = 0) => {
    ...
    return (
      <div key={folder.id}>
        ...
        >
          <span className="sb-folder-arrow" style={{ transform: folder.expanded ? 'rotate(90deg)' : undefined }}>▶</span>
          {renamingId === folder.id ? (
            <input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') handleRename(folder.id, 'folder'); if (e.key === 'Escape') setRenamingId(null) }}
              onClick={(e) => e.stopPropagation()}
              className="sb-rename-input"
            />
          ) : (
  ...
  return (
    <>
      ...
      <nav
        ...
        className="sb-panel"
        style={{
          width: isMobile ? 260 : 240, height: '100%', flexShrink: 0,
      ...
      >
        <div className="sb-header">
          <div className="sb-header-row">
            ...
            <button aria-label="关闭侧边栏" onClick={() => setSidebarOpen(false)} className="sb-close-btn">×</button>
          </div>
          <div className="sb-actions">
            <button onClick={() => createDoc()} className="sb-btn-new" aria-label="新建画布">+ 新画布</button>
            <button onClick={() => createFolder('新文件夹')} className="sb-btn-folder" aria-label="新建文件夹">+ 文件夹</button>
          </div>
        </div>
        ...
        <div className="sb-footer" aria-live="polite">{docs.length} 画布</div>
      </nav>
