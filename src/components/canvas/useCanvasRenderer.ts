...
import {
  drawElement, drawStrokeRaw, drawSelBox, drawMonetGrid,
  drawCanvasBackground, drawMinimap, drawZoomLevel,
...
export function useCanvasRenderer(
  ...
  function cachedBounds(el: CanvasElement) {
    ...
    if (!b) { b = elementBounds(el); cache.set(el.id, b) }
    return b
  }
  ...
  const getOrCreateEC = useCallback(() => {
    ...
    const tw = canvasSize.w * dpr, th = canvasSize.h * dpr
    if (ec.width !== tw || ec.height !== th) { ec.width = tw; ec.height = th; elementsDirtyRef.current = true }
    return ec
  }, [canvasSize, dpr])

  const renderElementsToCache = useCallback(() => {
    const ec = getOrCreateEC(); const ctx = ec.getContext('2d'); if (!ctx) return
    ...
    ctx.save(); ctx.scale(vb.zoom, vb.zoom); ctx.translate(-vb.x, -vb.y)
    drawMonetGrid(ctx, vb, canvasSize, dark)
    const vl = vb.x, vt = vb.y, vw = canvasSize.w / vb.zoom, vh = canvasSize.h / vb.zoom
    for (const el of els) {
    ...
    ctx.restore(); elementsDirtyRef.current = false
  ...
  const redraw = useCallback(() => {
    ...
    if (ds.tool === 'eraser' && ds.mousePos) {
      ...
      ctx.lineWidth = 1.5 / vb.zoom; ctx.setLineDash([5 / vb.zoom, 5 / vb.zoom])
      ctx.beginPath(); ctx.arc(ds.mousePos.x, ds.mousePos.y, r, 0, Math.PI * 2); ctx.stroke()
      ctx.setLineDash([])
      ctx.fillStyle = dark ? 'rgba(200,160,176,0.04)' : 'rgba(176,125,110,0.04)'
      ctx.beginPath(); ctx.arc(ds.mousePos.x, ds.mousePos.y, r, 0, Math.PI * 2); ctx.fill()
      ctx.restore()
    }
    ctx.restore()
    if (ds.marquee) {
      ...
      const sx = (x - vb.x) * vb.zoom, sy = (y - vb.y) * vb.zoom, sw = w * vb.zoom, sh = h * vb.zoom
      ctx.save()
      ctx.strokeStyle = dark ? 'rgba(200,160,176,0.7)' : 'rgba(176,125,110,0.7)'
      ctx.lineWidth = 1.5; ctx.setLineDash([5, 5]); ctx.strokeRect(sx, sy, sw, sh); ctx.setLineDash([])
      ctx.fillStyle = dark ? 'rgba(200,160,176,0.06)' : 'rgba(176,125,110,0.06)'; ctx.fillRect(sx, sy, sw, sh)
      ctx.restore()
    }
    ...
    if (snaps.x.length > 0 || snaps.y.length > 0) {
      ...
      for (const lx of snaps.x) { const sx2 = (lx - vb.x) * vb.zoom; ctx.beginPath(); ctx.moveTo(sx2, 0); ctx.lineTo(sx2, canvasSize.h); ctx.stroke() }
      for (const ly of snaps.y) { const sy2 = (ly - vb.y) * vb.zoom; ctx.beginPath(); ctx.moveTo(0, sy2); ctx.lineTo(canvasSize.w, sy2); ctx.stroke() }
  ...
  useEffect(() => {
    ...
    const obs = new ResizeObserver((entries) => {
      for (const e of entries) { const { width, height } = e.contentRect; if (width > 0 && height > 0) setCanvasSize({ w: Math.round(width), h: Math.round(height) }) }
    })
  ...
  useEffect(() => {
    ...
    const unsub = useAppStore.subscribe((s) => {
      if (s.elements !== prev) { elementsDirtyRef.current = true; boundsCacheRef.current.clear(); prev = s.elements }
      if (rafRef.current) cancelAnimationFrame(rafRef.current)
      rafRef.current = requestAnimationFrame(() => redrawRef.current())
    })
