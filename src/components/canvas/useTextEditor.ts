...
export function useTextEditor(canvasRef: React.RefObject<HTMLCanvasElement | null>) {
  ...
  const measureTextWidth = useCallback((content: string, fontSize: number): number => {
  ...
  const commitTextEdit = useCallback((content: string) => {
  ...
  const startEditText = useCallback(
    (x: number, y: number, screenX: number, screenY: number, color: string, existingEl?: { id: string; content: string; fontSize: number }) => {
      if (existingEl) {
        setEditingText({
          id: existingEl.id,
          x,
  ...
  return { editingText, setEditingText, textRef, measureTextWidth, commitTextEdit, startEditText, cancelEdit }
}
