...
describe('useAppStore', () => {
  ...
  it('should add a stroke element', () => {
    useAppStore.getState().addElement({ type: 'stroke', id: 's1', points: [[0, 0], [10, 10]], color: '#000', size: 2, brush: 'pen' })
  ...
  it('should add a shape element', () => {
    useAppStore.getState().addElement({ type: 'shape', id: 'sh1', kind: 'rectangle', x: 0, y: 0, w: 100, h: 50, color: '#000', size: 2 })
    expect(useAppStore.getState().elements).toHaveLength(1)
    expect(useAppStore.getState().elements[0].type).toBe('shape')
  })

  it('should add a text element', () => {
    useAppStore.getState().addElement({ type: 'text', id: 't1', x: 10, y: 10, width: 200, height: 30, content: 'Hello', fontSize: 16, color: '#000' })
    expect(useAppStore.getState().elements).toHaveLength(1)
    expect((useAppStore.getState().elements[0] as any).content).toBe('Hello')
  })

  it('should add an image element', () => {
    useAppStore.getState().addElement({ type: 'image', id: 'i1', x: 0, y: 0, width: 200, height: 200, dataUrl: 'data:image/png;base64,abc' })
    expect(useAppStore.getState().elements).toHaveLength(1)
  ...
  it('should move an element', () => {
    useAppStore.getState().addElement({ type: 'shape', id: 'sh1', kind: 'rectangle', x: 10, y: 20, w: 100, h: 50, color: '#000', size: 2 })
